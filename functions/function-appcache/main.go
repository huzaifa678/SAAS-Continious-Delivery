// Command function-appcache is a Crossplane composition function that composes
// the provider-aws ElastiCache resources for the platform.saas.example XAppCache
// API. It is the code form of what used to be the function-patch-and-transform
// YAML in platform/compositions/appcache-*.yaml — one function that branches on
// the claim's mode (shared | dedicated).
package main
...
func main() {
	ctx := kong.Parse(&CLI{},
		kong.Description("A Crossplane composition function for platform.saas.example XAppCache."),
		kong.UsageOnError())
